...
<head>
	...
	<script language="javascript">
		function prtReport() {
			transRateCountForm.action = '${pageContext.request.contextPath}/wctrans/download';
			transRateCountForm.submit();
			transRateCountForm.action = '${pageContext.request.contextPath}/wctrans/search';
		}
	</script>
</head>
<body>
	<div class="bjui-pageHeader">
		<form:form id="pagerForm" name="transRateCountForm"
			...
			action="${pageContext.request.contextPath}/wctrans/search"
			...
			<div class="form-group" style="margin: 15px 0 15px; ">
				...
				<form:select path="incomePlatform" data-toggle="selectpicker">
					...
					<form:option value="UB">微信</form:option>
					<form:option value="UC">支付宝</form:option>   
					<form:option value="UD">公众号</form:option> 
					<form:option value="UE">服务窗</form:option>	
				</form:select>
				<label>交易状态：</label>
			...
			<tbody>
				<c:forEach var="record" items="${pageUser.list}" varStatus="status">
					<tr data-id="<c:out value="${record.incomeId}"/>">
						...
						<td align="center">
							<c:if test="${record.incomeType == '03'}">微信</c:if>
							<c:if test="${record.incomeType == '04'}">支付宝</c:if>
							<c:if test="${record.incomeType == '07'}">公众号</c:if>
							<c:if test="${record.incomeType == '08'}">服务窗</c:if>
						</td>
						<td align="center"><c:out value="${record.incomePlatform}" /></td>
