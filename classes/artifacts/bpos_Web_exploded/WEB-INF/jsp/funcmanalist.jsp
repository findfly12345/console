    ...
    <form id="pagerForm" name="funcManageForm" data-toggle="ajaxsearch" action="${pageContext.request.contextPath}/funcManagement/query_a_func" method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <label>描述：</label>
            <input type="text" name="funcDesc" id="funcDesc" value="${funcManageForm.funcDesc}"  size="15">&nbsp;
            <select id="selectFuncDisableTag" name="funcDisableTag">
                <option value="" <c:out value="${funcManageForm.funcDisableTag==''?'selected':''}" />>--资源状态--</option>
                <option value="1" <c:out value="${funcManageForm.funcDisableTag=='1'?'selected':''}" />>启用</option>
                <option value="0" <c:out value="${funcManageForm.funcDisableTag=='0'?'selected':''}" />>禁用</option>
            </select>
