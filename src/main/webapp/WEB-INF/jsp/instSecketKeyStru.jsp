...
<head>
    ...
    <script>
    	//表单提交，验证参数
            function generateSecretKey(instCode){
    		if (!confirm("确认要生成密钥?")) {
               return false;
            }
    		var url="${pageContext.request.contextPath}/IM/generateInstSecKey";
    		var index =document.getElementsByName("instSecKeys["+ instCode+"].instHsmIndex");
    		var instHsmIndex = index[0].value;
    		if(instHsmIndex == null || instHsmIndex == "" ) {
    	        	art.dialog({
            			id : 'dialog',
                        icon: 'error',
                        content: '请输入密钥索引'
    	    ...
    	    $.post(url,{instCode:instCode,instHsmIndex:instHsmIndex,index:instCode, dataType:"json"},callBackGenerateSK);

    	}

    	function callBackGenerateSK(data,textStatus){
    		if(data!=null){
    			if( data.res != null && data.res.indexOf("HTS1111")>=0){
    				alert('生成密钥失败!');
    			}else if(data.res != null && data.index != null)
    				{ alert('生成密钥成功!');
    				$(".content.current font[id='"+data.index+"']").html("已生成密钥")}
    			else {alert('生成密钥失败!');}
    		}else{
    			alert('生成密钥失败!');
    		}
...
<div class="bjui-pageHeader">
    <form id="pagerForm" name="instTransCtrlPerform" data-toggle="ajaxsearch"   action="${pageContext.request.contextPath}/IM/struSecretKey" method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <label>机构号：</label>
            <input type="text" name="instId" id="instId" value="${instTransCtrlPerform.instId}"  size="15">&nbsp;
            <label>机构类别：</label>
...
<tr data-id="<c:out value="${record.instId}"/>">
      ...
      <td align="center">
          <a href="${pageContext.request.contextPath}/IM/generateInstSecKey?id=<c:out value="${record.instId}"/>" class="btn btn-red" data-toggle="doajax" >
              ...
              <c:if test="${record.checkStatus!='Y'}">点击签到</c:if>
          </a>
