...
<div class="bjui-pageHeader">
    <form id="pagerForm" name="terminalForm" data-toggle="ajaxsearch" action="${pageContext.request.contextPath}/terminal/bind_terminal_list" method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <select name="termStat" id="termStat" data-toggle="selectpicker">
                ...
                <option value="1" <c:if test="${record.termStat=='1'}">selected</c:if>>已开通</option>
                <option value="0" <c:if test="${record.termStat=='0'}">selected</c:if>>未开通</option>
            </select>&nbsp;
            ...
            <tr data-id="<c:out value="${record.termId}"/>">
                ...
                <td align="center">
                    <c:if test="${record.bindStat=='0'}">
                        <a href="${pageContext.request.contextPath}/terminal/go_to_bind_terminal?termId=${record.termId}" class="btn btn-green" data-toggle="navtab" data-id="form-viewagtinfo"  data-title="绑定商户">绑定商户</a>
                    </c:if>
                    <c:if test="${record.bindStat=='1' || record.bindStat == null}">
                        <a href="javascript:;" class="btn btn-default" disabled="true" data-title="已绑定">已绑定</a>
                    </c:if>
                </td>
            </tr>
        </c:forEach>
