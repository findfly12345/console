...
<div class="bjui-pageHeader">
    <form id="pagerForm" name="merInfoForm" data-toggle="ajaxsearch" action="${pageContext.request.contextPath}/mer/agent_mer_image_list" method="post">
    ...
    <table  class="table table-bordered table-hover table-striped table-top" data-toggle="tablefixed" data-width="100%" data-nowrap="true">
        ...
        <c:forEach var="record" items="${merInfoForm.pagination.list}" varStatus="status">
            <tr data-id="<c:out value="${record.merId}"/>">
                ...
                <td align="center">
                    <c:if test="${record.merStat !='0' && record.merStat !='4'}">
                        <a href="${pageContext.request.contextPath}/mer/go_to_upload_image_page?merId=<c:out value="${record.merId}"/>&agentId=<c:out value="${record.agentId}"/>" class="btn btn-green" data-toggle="navtab"data-id="navtab-detail"  data-title="完善商户图片附件(${record.merName})">完善商户照片附件</a>
                    </c:if>
